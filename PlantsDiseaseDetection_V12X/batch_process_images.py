...
# Fix import path to be absolute for when it's imported from outside this directory
import os
import sys
...
try:
    # Try relative import first (when running from inside the package)
    from .YOLOv12XModel import YOLOv12XDetector, ini_params
except ImportError:
    # Fall back to absolute import (when imported from elsewhere)
    from YOLOv12XModel import YOLOv12XDetector, ini_params
...
def setup_model(weights_path=None, conf_threshold=0.25, iou_threshold=0.5):
    """
    Initialize and configure the YOLOv12X model.
    ...
    """
    # Initialize model
    model = YOLOv12XDetector()
    
    # Load model weights
    if weights_path is None:
        weights_path = abs_path("weights_v12/best.pt", path_type="current")
...
def main():
    ...
    parser.add_argument('--weights', '-w', help='Path to model weights file (default: weights_v12/best.pt)')
