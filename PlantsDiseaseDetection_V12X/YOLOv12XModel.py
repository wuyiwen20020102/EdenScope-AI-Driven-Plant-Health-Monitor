...
from datasets.PlantsDisease.label_name import Chinese_name  # 从datasets库中导入Chinese_name字典，用于获取类别的中文名称

# Try to import ultralytics, install newer version if needed
try:
    # First try import to see if current ultralytics works
    from ultralytics import YOLO
    from ultralytics.utils.torch_utils import select_device
    
    # Test if it has the necessary modules for YOLOv12X
    import inspect
    from ultralytics.nn.modules import block
    if not hasattr(block, 'C3k2'):
        raise AttributeError("Current ultralytics version doesn't support YOLOv12X")
        
except (ImportError, AttributeError) as e:
    print(f"Updating ultralytics for YOLOv12X support: {e}")
    subprocess.check_call([sys.executable, "-m", "pip", "install", "ultralytics>=8.2.0", "--upgrade"])
    
    # Reload modules to use the newly installed version
    if "ultralytics" in sys.modules:
        del sys.modules["ultralytics"]
        
    # Import again using the updated package
    from ultralytics import YOLO
    from ultralytics.utils.torch_utils import select_device
...
class YOLOv12XDetector(Detector):  # 定义YOLOv12X检测器类，继承自Detector类
