...
from YOLOv12XModel import YOLOv12XDetector  # 从YOLOv12XModel模块导入YOLOv12XDetector类，用于物体检测
...
class MainWindow(QMainWindow):  # 定义MainWindow类，继承自FBaseWindow类
    def __init__(self):  # 定义构造函数
        ...
        self.label.setGeometry(0, 0, 640, 640)  # 设置QLabel对象的几何形状
...
def frame_process(image):  # 定义frame_process函数，用于处理每一帧图像
    ...
    print("Total time: %.2f" % use_time)  # 打印预测所用的时间
    det = pred[0]  # 获取预测结果
    # 如果有检测信息则进入
...
model = YOLOv12XDetector()  # 创建YOLOv12XDetector对象
model.load_model(abs_path("weights_v12/best.pt", path_type="current"))  # 加载预训练的YOLOv12X模型
